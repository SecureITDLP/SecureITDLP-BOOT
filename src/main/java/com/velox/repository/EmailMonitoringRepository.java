...
public interface EmailMonitoringRepository extends JpaRepository<EmailMonitoringEntity, Long> {
	@Query(value = "SELECT d.date AS incident_date, COUNT(e.date_time) AS count " + "FROM ( "
			...
			+ "    UNION ALL SELECT CURDATE() " + ") d " + "LEFT JOIN email_monittoring e "
			+ "    ON e.date_time >= d.date " + "   AND e.date_time < d.date + INTERVAL 1 DAY " + "GROUP BY d.date "
