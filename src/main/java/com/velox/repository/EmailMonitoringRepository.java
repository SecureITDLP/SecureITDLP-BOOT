...
public interface EmailMonitoringRepository extends JpaRepository<EmailMonitoringEntity, Long> {
	@Query(value = 
		    "SELECT d.date AS incident_date, COUNT(e.date_time) AS count " +
		    "FROM ( " +
		...
		@Query("SELECT new com.velox.dto.EmailModalDto(" +
			       "e.sender, e.receiver, e.hostName, e.attachments, e.subject) " +
			       "FROM EmailMonitoringEntity e " +
