...
public interface NetworkDlpRepository extends JpaRepository<NetworkDlpEntity, Long> {
	...
	@Query("SELECT new com.velox.dto.EventTypeCountDto(n.eventType, COUNT(n)) FROM NetworkDlpEntity n GROUP BY n.eventType")
	...
	List<ExtensionCountDto> countByExtension();
	 ...
	 @Query("SELECT new com.velox.dto.PeripheralCountDto(n.eventType, COUNT(n)) FROM NetworkDlpEntity n WHERE n.eventType IN ('USB Transfer', 'DVD Burn') GROUP BY n.eventType")
